...
public class ListingEndpoint {
    ...
    @GetMapping("listing/ByCategory/{id}")
    public ResponseEntity<Listing> listingByEmail(@PathVariable("id") int id) {
        Optional<Listing> listing = listingRepasitory.findListingByCategoryId(id);
        if (listing.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(listing.get());
